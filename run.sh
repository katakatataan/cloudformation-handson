...
KEY_PAIR_NAME=cloudformation-handson-matsuyay
ENVIORNMENT_NAME=dev
PROFILE=cloudhandson
# かぶらないもの
APP_S3_BUCKET=app-s3-bucket-cloudformation-hands-on-api
APP_S3_KEY=sample.zip

SERVICE_NAME=api
PRIVATE_DOMAIN_NAME=private-cloudformation-handson.com
DOMAIN_NAME=public-cloudformation-handson.com
DB_DOMAIN_NAME=db.${PRIVATE_DOMAIN_NAME}
# かぶらないもの
CLOUDFORMATION_TEMPLATE_BUCKET=yyyyyyy-cloudformation-hands-on-resource

if aws s3 ls "s3://$CLOUDFORMATION_TEMPLATE_BUCKET" --profile $PROFILE --region ap-northeast-1 2>&1 | grep -q 'An error occurred'; then
...
#  vpc
#----------------------------------------------------------
if aws cloudformation describe-stacks --profile $PROFILE --region ap-northeast-1 --stack-name ${PROJECT_NAME}-${ENVIORNMENT_NAME}-vpc ; then
  # すでに存在するものへの変更
...
#  dns depends on vpc
#----------------------------------------------------------
if aws cloudformation describe-stacks --profile $PROFILE --region ap-northeast-1 --stack-name ${PROJECT_NAME}-${ENVIORNMENT_NAME}-dns ; then
  # すでに存在するものへの変更
  aws cloudformation deploy \
...
#  db dependds on db
#----------------------------------------------------------
if aws cloudformation describe-stacks --profile $PROFILE --region ap-northeast-1 --stack-name ${PROJECT_NAME}-${ENVIORNMENT_NAME}-db ; then
  # すでに存在するものへの変更
  aws cloudformation deploy \
  ...
  aws cloudformation create-stack \
    ...
    --profile $PROFILE \
    --parameters \
    ParameterKey=DBDomainName,ParameterValue=$DB_DOMAIN_NAME \
    ParameterKey=ProjectName,ParameterValue=$PROJECT_NAME \
...
aws cloudformation package \
  ...
  --profile $PROFILE
if aws cloudformation describe-stacks --profile $PROFILE --region ap-northeast-1 --stack-name ${PROJECT_NAME}-${ENVIORNMENT_NAME}-${SERVICE_NAME} ; then
  # すでに存在するものへの変更
  aws cloudformation deploy \
